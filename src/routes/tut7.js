...
const TutSevenQuestions   = require('../resources/TutSevenQuestions');

const ACTIVITY_NAME = "tut7";
...
module.exports = function (app) {
    ...
    app.get(absRoute(`/${ACTIVITY_NAME}/questions`), [ensureAuthenticated, withErrorHandler(async (req, res) => {
        ...
        res.render(`${ACTIVITY_NAME}/questions`, {
            session,
            questions: TutSevenQuestions,
            absRoute
        });
    })]);

    // Define routes for all questions in resource file.
    for (let i = 0; i < TutSevenQuestions.length; i++) {
        const question = TutSevenQuestions[i];
