...
const TutFiveQuestions    = require('../resources/TutFiveQuestions');

const ACTIVITY_NAME = "tut5";
...
module.exports = function (app) {
    ...
    app.get(absRoute(`/${ACTIVITY_NAME}/questions`), [ensureAuthenticated, withErrorHandler(async (req, res) => {
        ...
        res.render(`${ACTIVITY_NAME}/questions`, {
            session,
            questions: TutFiveQuestions,
            absRoute
        });
    })]);

    // Define routes for all questions in resource file.
    for (let i = 0; i < TutFiveQuestions.length; i++) {
        const question = TutFiveQuestions[i];
