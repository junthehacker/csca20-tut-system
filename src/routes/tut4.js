...
const TutFourQuestions    = require('../resources/TutFourQuestions');

const ACTIVITY_NAME = "tut4";
...
module.exports = function (app) {
    ...
    app.get(absRoute(`/${ACTIVITY_NAME}/questions`), [ensureAuthenticated, withErrorHandler(async (req, res) => {
        ...
        res.render(`${ACTIVITY_NAME}/questions`, {
            session,
            questions: TutFourQuestions,
            absRoute
        });
    })]);

    // Define routes for all questions in resource file.
    for (let i = 0; i < TutFourQuestions.length; i++) {
        const question = TutFourQuestions[i];
